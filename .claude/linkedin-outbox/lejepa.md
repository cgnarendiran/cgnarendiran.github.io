...
LeJEPA swaps the lot for one term. Cast a part before anyone tells you what it bolts to and you pour it equally strong in every direction. The load comes from an angle you did not pick. Embeddings, same. The shape is an isotropic Gaussian. You cannot X-ray an ingot, so it core-samples: random directions, test each shadow.
