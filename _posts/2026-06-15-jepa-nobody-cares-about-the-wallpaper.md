...
Her description throws away almost everything she saw. The wallpaper, the carpet, the light through the blinds, the pattern on the coffee cup. All gone, and none of it missed, because none of it was ever going to help you find anybody.
...
Great trick. It gave us BERT. The trouble starts when you ask *what* the model should fill the blank in with.
...
So the model spends its capacity on the least informative part of the image. Not because anyone designed it that way, but because pixels are where the entropy lives, and squared error goes looking for entropy.

Video makes this worse rather than better, which is unfortunate, because video is the biggest pile of unlabeled data we have. A photo has no future to get wrong. A clip does, and most of what happens in the next frame is unknowable at the pixel level even if you understand the scene perfectly.

Ask for the description instead.
...
Now notice what that costs, because it's the whole rest of this post.
...
Zero loss, global minimum, nothing learned. This is **representation collapse**, and it is the reason joint-embedding methods were considered cursed for years. Reconstruction never had this problem, because you cannot fake a photograph. You can absolutely fake a description.
...
with the decay $\tau$ usually somewhere around 0.996 to 0.999. The target updates slowly, so the two can never quite settle on a shared story. [SimSiam](https://arxiv.org/abs/2011.10566) throws out the EMA and keeps the stop-gradient. [DINO](https://arxiv.org/abs/2104.14294) runs teacher-student distillation with centering pulling the output toward uniform and sharpening pulling it toward peaked, the two balanced by a schedule that somebody tuned.

Here is the part I find genuinely uncomfortable. **None of this forbids collapse.** The constant solution is still a perfectly valid global minimum of BYOL's loss. It just doesn't happen, for reasons that took a small literature of follow-up papers to half-explain. We built foundation models on top of that.
...
These actually rule out the constant solution, no teacher required. They deserve more credit than they got. Hold that thought, it becomes the entire story in part 2.
...
A world model needs a transition function, $s_{t+1} = f(s_t, a_t)$. Given where you are and what you *do*, where do you end up. Go back and look at what the JEPA predictor is conditioned on. Position. It answers "what's over there", not "what happens if I try this".
...
Optimize with the cross-entropy method, execute the first action, replan. All of it in latent space. No frame is ever drawn.

Deployed zero-shot on Franka arms in two labs with no data from either: roughly 80% pick-and-place, 75% reach-with-object, 65% grasp on cups and boxes. Octo manages 15% on grasp. Planning takes **16 seconds per action** against about 4 minutes for Cosmos, which has to generate pixels in order to think.

And the line that settles it is in the ablation: action-unconditioned variants fail to infer control. Same encoder, same data, same everything else. Take away the action conditioning and the world model stops being one. Tier 2 is not a smaller tier 3.

## Why this matters

The useful lesson here is that the objective was the bottleneck, not the architecture. Same transformers, same data, same compute. Change what you ask the model to predict and the representations get better and cheaper at once.
...
But look again at what's holding it up. An EMA copy, a stop-gradient, a predictor hobbled on purpose. Every one of those exists to answer a question nobody could answer directly: what stops the witnesses from agreeing on nothing?

For five years the field could only say what a bad description looks like. Nobody could write down what a good one should look like.

In late 2025, somebody did, and they brought a proof.
