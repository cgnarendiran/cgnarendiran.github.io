...
The tie is the result. Half the parameters, a third of the epochs, and the entire apparatus of part 1 removed: no teacher, no stop-gradient, no predictor, no schedules. Scale it up and a ViT-H/14 gets 79% on ImageNet-1K under a frozen linear probe, against DINOv2's ViT-g/14 at 86.5%. Seven and a half points is not a rounding error and nobody pretends it is.

The number I would actually build on is a different one. The training loss correlates with downstream probe accuracy at **94%+ Spearman**. You can pick a model by looking at the loss. No labels, no held-out probe, no waiting three days to find out the run was dead on Tuesday. After five years of a number on a screen that told you nothing, somebody wired the gauge to the tank.

Which drops the price of admission. Their worked example gets a ViT-S/8 to 90.7% on Imagenette in about 200 minutes on one GPU. And in-domain pretraining stops being a doomed move: on Galaxy10, roughly 11k pictures of galaxies, training from scratch beats transferring from DINOv2. Eleven thousand images, against a model that ate the internet.
...
**Arguing with the target**: the isotropic Gaussian is now something people push back on, which is the healthiest possible sign. [Beyond Isotropy in JEPAs](https://arxiv.org/abs/2605.20107) proposes a Hamiltonian geometry instead, SPHERE-JEPA goes to the sphere, KerJEPA swaps the characteristic-function test for kernel discrepancies. People arguing about whether the target is right is a much better problem to have than the previous one, where there was no target to argue about and everyone just added another trick.
