...
Hold on to those four numbers, because there is already a mistake in them, and it is the same mistake the next two days of argument were built on. They count *orders in which moves can be played*, not distinct arrangements of the board. Play 1.Nf3 Nf6 2.Ng1 Ng8 and both sides are back exactly where they started, having achieved nothing whatsoever. That counts as a fresh sequence. It is plainly not a fresh board.
...
So one side was counting sequences, the other was counting arrangements, and both were sure the other was innocent of arithmetic. Nobody in six million views said the thing that actually settles it.
...
The root is the starting position. Every edge is a legal move. Every node is the position you arrive at. A **game** is one path from the root down to a leaf where somebody is mated or the draw rules bite. A **position** is a node.

Paths outnumber nodes, and by a lot, for one reason. Different move orders land on the same arrangement of pieces, so many paths run through the same node. Chess programmers call this a **transposition**, and they call those counts from the opening post **perft**, short for performance test, used to check that a move generator has not lost its mind. The [wiki that publishes them](https://www.chessprogramming.org/Perft) says outright that what is counted is move paths. The tree double-counts what the board does not.
...
Chess.com's forty orders of magnitude above the atoms is right, for paths. Musk's forty below is roughly right, for nodes; the real gap is nearer 36. Two accurate numbers describing two different objects, fired at each other for two days.
...
The two are not even the same kind of number. Shannon's $10^{120}$ is a napkin, from [Programming a Computer for Playing Chess](https://vision.unipv.it/IA1/ProgrammingaComputerforPlayingChess.pdf) in 1950: take forty moves a side and about thirty options each, so a move-pair contributes $30 \times 30 \approx 10^3$, and forty of those gives $(10^3)^{40} = 10^{120}$. That is the entire derivation. No error bar, because it was never meant to carry one. He wanted to show brute force was hopeless, and it was.

The node count is a real measurement, and it is the better story.

## Counting nodes nobody has

You cannot count distinct chess positions by listing them. Nobody has the list and nobody will. So John Tromp did something sneakier.

Think of measuring a lake with a wiggly shoreline. Draw a rectangle around it whose area you know exactly, throw darts uniformly, and see what fraction land in water. Five percent wet means the lake is five percent of the rectangle. You never traced the coast.
...
One honest caveat before anyone quotes that at a party. Complexity classes describe families that grow with input size. Standard chess is 8x8 and finite, so formally it is a constant, solvable in $O(1)$ by a lookup table you would merely have to build first. The theorem says the difficulty is structural rather than a failure of imagination. It does not say 8x8 chess is intractable.

## What "solved" actually means

Now the part that decides the argument, and it is not about size at all.

Solving does not mean finding the line both sides play when both play perfectly. That is one path. It would be worthless the instant your opponent stepped off it.

A solution is a **strategy**, and it has to survive anything the other side does. Two grades of it:
...
Branch wide on their turns, branch by one on yours. That single asymmetry is why solving anything is possible at all.
...
Worth feeling the exponents. Chess has $b \approx 35$ and a game runs about 80 ply, a ply being one move by one side. The full tree is $35^{80} \approx 10^{123}$, which is Shannon's number arrived at from a different direction. The square root is $35^{40} \approx 10^{62}$. Sixty orders of magnitude off the top, for free, from one observation about whose turn it is.
...
The number to walk away with: proving it took about $10^{14}$ nodes. One in five million. The rest was proved irrelevant and discarded, using proof-number search, an idea from [Victor Allis's 1994 thesis](http://fragrieu.free.fr/SearchingForSolutions.pdf) that measures how many nodes a proof still needs.
...
The obvious objection, and it deserves a real answer, because [AlphaZero](https://www.science.org/doi/10.1126/science.aar6404) is one of the better things to happen to this field.
...
The contrast in the paper is lovely: AlphaZero examined about 60,000 nodes a second against Stockfish's 60 million, and won. A thousandth as many, because they were better ones.
...
And that is exactly why it is not a solution. Every leaf of that search bottoms out in a guess from a learned function. Guesses can be wrong, and nothing inside the system can tell you which ones are. Being unbeaten is not the same as being checked.
...
**Size.** Each extra piece multiplies the count by roughly a thousand, because the new piece can stand anywhere in every arrangement you already had. Seven pieces is 18.5 TB in the compact Syzygy encoding and was 140 TB in the original Lomonosov format. Eight lands in the petabytes.

**Dependency.** These are built backwards from checkmate, so a node's value needs the values of everything it can turn into. Captures drop you into a smaller material set. Promotions drop you into a different one. Every eight-piece file sits on a stack of seven-piece files that must be finished first, and pawnless combinations go first precisely because promotion is not in their dependency graph.

## Suppose somebody handed you every node

The algorithm is not the hard part. It is four lines, and it is what tablebase generation already does:
...
Three things kill it, and cleverness is not among them.

Storage first. One label per node at two bits is about $2.5 \times 10^{43}$ bytes, against total human storage somewhere around $10^{23}$. Short by twenty orders of magnitude.

Then the access pattern, which people forget. This is not a stream you can chew through in chunks. Each sweep needs the predecessors of nodes scattered anywhere in the array, so it is random access across the whole thing. That is already the binding constraint at seven pieces, where the bottleneck is I/O and not arithmetic.

Then thermodynamics, for fun. Landauer's limit puts the cost of erasing a bit at $kT \ln 2 \approx 2.9 \times 10^{-21}$ J at room temperature. Writing $10^{44}$ bits once, at the floor, with zero overhead, is around $3 \times 10^{23}$ J. A few centuries of total human energy production, for one pass, and forced wins in the seven-piece tables already run past 500 moves. (That arithmetic is mine. Treat it as a sanity check, not a citation.)
...
Where it runs aground is his next sentence, the one about a superintelligence finding compressions we could not follow. Pruning has to be **proved** safe, not felt to be safe, and we have exactly one sound pruning rule for game trees. Alpha-beta: proposed by John McCarthy at the Dartmouth workshop in 1956, written up by Hart and Edwards in 1961, published by Brudno in 1963, analysed by Knuth and Moore in 1975. It discards edges that are provably irrelevant given the values already in hand. Everything a modern engine layers on top, null-move, late-move reductions, futility pruning, is a heuristic. Which is why Stockfish is terrifying and why it proves nothing: every reduction is a bet that a branch was not worth walking.

Seventy years, one sound rule. That is a mathematical gap, and mathematical gaps do not close on a compute curve. Maybe a machine finds the second rule. There is no evidence it will and none that it won't, which is a different thing from confidence, and both sides of that thread were performing confidence.

So the tree is not the obstacle. The tree was never the obstacle. Shannon knew in 1950 that you would never walk it, and everything since has been about deciding which parts you are allowed to skip. Chess stays unsolved not because the universe is too small to hold the answer, but because nobody can yet prove which branches they never had to look at.
